...
void CBEBouquetWidget::paintItem(int pos)
{
	...
	if(current < Bouquets->size()) {
		...
		g_Font[SNeutrinoSettings::FONT_TYPE_CHANNELLIST]->RenderString(x+iconoffset+20, ypos + iheight - (iheight-fheight)/2, width-iconoffset-20, (*Bouquets)[current]->lName, color);
	}
}
